...
(function(){
	...
	function Main(cfg){
		...
		this.config = {
			...
			dragAndDropTips: cfg.dragAndDropTips || "<span>或者把文件(文件夹)拖拽到这里</span>",
			fileFieldName : cfg.fileFieldName || "FileData",
			browseFileId : cfg.browseFileId || "i_select_files",
			browseFileBtn : cfg.browseFileBtn || "<div>请点我选择文件</div>",
	...
	Main.prototype = {
		...
		onSelect : function(list) {
			fShowMessage("你已选择 " + list.length+"个文件！！");
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage("File counts:" + selected + ", but limited:" + limit, true);
		},
		onMaxSizeExceed : function(file) {
			fShowMessage("你的文件:" + file.name + " size is:" + file.size +" Exceed limited:" + file.limitSize, true);
		},
		...
		onCancel : function(file) {
			fShowMessage("你已删除： " + file.name);
		},
		onStop : function() {
			fShowMessage("你已暂停！！!");
		},
		onCancelAll : function(numbers) {
			fShowMessage( " 你的"+ numbers +"个文件已被你取消上传！！！！! ");
		},
		onComplete : function(file) {
			fShowMessage("你的文件:" + file.name + ", 大小（字节）:" + file.size + " 上传成功！！！");
		},
		onQueueComplete : function(msg) {
			fShowMessage("你上传队列中的文件：(服务器返回信息: "+msg+")	---==>	上传成功！！！！");
		},
		onUploadError : function(status, msg) {
			fShowMessage("Error Occur.  Status:" + status + ", 服务器返回信息: " + msg, true);
		},
		disable : function() {
