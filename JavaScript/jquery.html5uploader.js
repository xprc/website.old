...
$.fn.html5uploader = function(opts){
	...
	var defaults = {
		...
		buttonText:'点我选择文件',//上传按钮上的文字
		removeTimeout: 1000,//上传完成后进度条的消失时长
		itemTemplate:'<li id="${fileID}file"><div class="progress"><div class="progressbar"></div></div><span class="filename">${fileName}</span><span class="progressnum">0/${fileSize}</span><a class="uploadbtn">点我开始上传</a><a class="delfilebtn">点我删除本文件</a></li>',//上传队列显示的模替,最外层标签使用<li>
		onUploadStart:function(){},//上传开始时的动仿
		onUploadSuccess:function(){},//上传成功的动仿
		onUploadComplete:function(){},//上传完成的动仿
		onUploadError:function(){}, //上传失败的动仿
		onInit:function(){},//初始化时的动仿
		}
	...
	this.each(function(){
		...
		//创建文件对象
           ...
           _this.find('.uploadbtn').die().live('click',function(){
			  ...
			  onUploadSuccess: option.onUploadSuccess,		//文件上传成功敿
			  onUploadError: option.onUploadError,		//文件上传失败敿,
			  onUploadComplete: option.onUploadComplete,		//文件全部上传完毕敿
			  ...
			  funDealFiles: function(files) {
				  var fileCount = _this.find('.filelist li').length;//队列中已经有的文件个擿
